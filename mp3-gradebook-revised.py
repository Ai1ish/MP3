...
def add_student():
    ...
    print("\nStudent added!") 
    print("ID:", next_student_id, "Name:", name)
    next_student_id += 1
...
def add_category():
    ...
    if name == "":
        print("Name cannot be empty.")
        return
    current_total = sum(cat["weight"] for cat in categories.values())
    remaining = 100 - current_total
    if remaining <= 0:
        print(f"\nCannot add a new category. Total weight is already at 100%. Edit existing category weights first.")
        return
    print(f"Remaining weight available: {remaining}%")
    ...
    if current_total + weight > 100:
        print(f"\nWeight exceeds the remaining {remaining}%. Category not added.")
        return
    categories[next_category_id] = {"name": name, "weight": weight}
    new_total = current_total + weight
    print("\nCategory added!") 
    print("ID:", next_category_id, " Name:", name, " Weight:", weight, "%")
    print(f"Total weight used: {new_total}% / 100%")
    next_category_id += 1
...
def add_assignment():
    ...
    print("\nAssignment added!") 
    print("ID:", next_assignment_id, " Name:", name, " Max Points:", max_points)
    next_assignment_id += 1
...
def main():
    while True:
        ...
        match input("Enter option: "):
            ...
            case "0":
                verif = input("Are you sure you want to exit [y/n]: ").strip().lower()
                if verif == 'y': 
                    print("Thanks. Program exiting...")
                    break
                else: 
                    print("Exit has been cancelled.") 
            case _: print("Invalid option.")
